...
При отмене платформа очищает движения, устанавливает `posted=false` и вызывает
`OnUnpost` / `ОбработкаУдаленияПроведения` из модуля документа. Все три шага
выполняются в одной транзакции: исключение из DSL-хука откатывает отмену
проведения и возвращается как ошибка `422`.
...
по имени. Служебные параметры `limit` и `page` задают страницу плоского ответа:
`limit` по умолчанию равен 100 и ограничен 1000, `page` нумеруется с единицы.
Для стабильного обхода страниц запрос отчёта должен содержать устойчивый
`УПОРЯДОЧИТЬ ПО`; без него СУБД не гарантирует одинаковый порядок строк.
...
`meta.total` учитывает полный результат отчёта, включая применённые параметры,
права на строки и собственный `ПЕРВЫЕ` в запросе. `meta.truncated` сохраняется
для совместимости и равен `true`, если после текущей страницы остаются строки.
...
композицию. Композиция всегда строится по полному результату и не разбивается
на страницы: `limit` и `page` в этом режиме игнорируются, `meta.page` равен 1,
`meta.limit` и `meta.total` равны числу исходных строк, `meta.total_pages` равен
1 для непустого результата (0 для пустого), `meta.truncated` равен `false`.
Графики и пользовательские сохранённые настройки остаются UI-представлением.
