...
2. Кандидаты (если номер не задан и доработок нет): открытые ишью с меткой
   `ready-fix` **или** `approved`, минус `hold`, минус `needs-decision`;
...
7. Коммит `тип(scope): описание` по-русски с трейлером
   ...
   заголовок коммита; в теле — что сделано, почему так, спорные решения, и
   **обязательно** английское `Fixes #<N>`.
...
8. **Доработка PR по ревью** (пришёл сюда из п. 1):
   ...
   - прочитай последнее заключение (`<!-- pp:review -->`) — блокирующие
     замечания перечислены там;
   ...
   - правь **только по блокирующим** замечаниям; необязательные — на твоё
     усмотрение, но объём PR не расширяй: чужие находки по дороге — отдельная
     заявка, а не довесок;
...
9. Не получилось (не воспроизводится, нужен выбор, фикс выходит за рамки) —
   ...
   `in-work`, если ставил, worktree убрать, недоделанное не пушить.
...
Дальше по конвейеру: `/review-pr` пишет заключение и ставит `reviewed` либо
