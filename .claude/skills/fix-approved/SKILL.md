...
выбирает вариант и не меняет владельца мяча; игнорируй его как управляющее
событие и упомяни в итоговой сводке. Сам комментарий всё равно остаётся частью
полного issue/PR snapshot: если он появился после сохранённого fingerprint,
обычный mutation gate закрывается, но решение из него доверенным не становится.
...
1. **Сначала доработки.** Объедини два списка: PR с `changes-requested` и PR с
   ...
   пагинированным REST, примени единый trust predicate
   `author.login == "ivanarama"` и упорядочь по `created_at`, затем `id`. Для
   текущего SHA построй единый поток переходов владельца. В него
...
3. Прочитай заявку и каноничный триаж-комментарий — план фикса там. Триажем
   ...
   **Заявка сделана планом, а плана нет — не бери её.** Если разбор или
   ...
   или «планом N»), а такого файла в `Plans/` не лежит, план ещё не написан:
   срезов нет, границы не проведены, и твой PR ляжет мимо будущего плана. Это
   случай п. 9 — вопрос в заявку, `needs-decision`, снять
   `approved`/`ready-fix`.
