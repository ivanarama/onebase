...
1. **Сначала доработки.** Объедини два списка: PR с `changes-requested` и PR с
   ...
   **До CAS-push** продолжать можно, только пока HEAD совпадает с исходной canonical completion,
   ...
   его результат и сохрани фактически отправленный SHA. Успешный собственный
   push потребляет старое владение FIX: для разрешённых post-push шагов
   (комментарий о доработке, затем снятие `changes-requested`) уже не требуй
   равенства старому HEAD. Перед каждым из них перечитай HEAD/comments/labels и
   требуй HEAD == отправленному SHA, отсутствие `ship`/`hold`/`needs-decision` и
   отсутствие нового `pp:review-again`/completion после push. Иное состояние
   останавливает финализацию; MERGE/REVIEW восстановят маршрут нового HEAD.
...
4. Рабочее место (main занят другим worktree — локально его не трогать). Для
   ...
   суффикса. Сохрани SHA `origin/main` и **до начала работы** атомарно создай
...
8. **Доработка PR по ревью** (пришёл сюда из п. 1):
   ...
   - те же проверки, что в п. 6. Непосредственно перед push перечитай HEAD,
     ...
     completion или чужом push ничего не
