...
3. По состоянию:
   ...
   - **DIRTY (конфликт)** → чинить в отдельном worktree, привязанном к SHA
     ...
     **«Взять обе стороны» — проверяемое утверждение, а не намерение.** Перед
     commit сравни множества новых строк каждой стороны относительно
     merge-base с результатом: пропала хоть одна — верни эскалацию. Записи не
     переписывай и не переупорядочивай. Общую строку, которую обе стороны
