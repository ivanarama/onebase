...
`ship` — разрешение человека, но `hold` и `needs-decision` старше него. Это
обязательный гейт **твоей** очереди, а не запрет на уровне репозитория: защита
ветки ревью не требует, поэтому человек может влить PR руками и без метки. Тебе
это ничего не разрешает — железное правило выше действует независимо от того,
что можно сделать кнопкой. Метки ревью (`reviewed`, `changes-requested`) для
тебя информационные: если человек поставил `ship` на PR с
`changes-requested` и стопов нет, значит он так решил —
