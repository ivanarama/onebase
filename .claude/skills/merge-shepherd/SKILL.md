...
5. Мерж: `gh pr merge <N> --merge --delete-branch`. Отказался (статус успел
   ...
   **Влил PR, добавивший `Plans/NNN-*.md` — напомни про заявки на паузе.**
   Найди открытые заявки, которые ссылаются на этот план
   (`gh issue list --state open --search "<имя файла плана>" --json number,labels`),
   и у тех, что носят `hold`, оставь комментарий:

   ```
   План `Plans/NNN-*.md` влит (#<M>) — пауза «делаем планом» кончилась.
   Снимите `hold` и верните `approved` ведущей заявке, чтобы она поехала.
   ```

   Метки при этом **не трогай**: `approved` ставит только человек, а `hold`
   снимает он же. Твоё дело — сказать, что момент наступил, иначе он не
   наступает никогда: пауза оформлена, ссылка живая, и заявка выглядит
   образцовой, пока её не хватятся. Ровно этот разрыв — между «план влит» и
   «метку вернули» — и остаётся единственным местом, где конвейер ждёт памяти
   человека. Вторая страховка на тот же случай — корзина «hold, а план уже
   влит» в `tools/backlogsweep`.
