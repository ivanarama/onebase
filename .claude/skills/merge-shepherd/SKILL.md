...
Железное правило: вливаются **только** PR с меткой `ship` (её ставит Иван) и
без `hold`. PR без `ship` не трогать вообще — ни обновлять, ни комментировать.

## Процедура

1. Очередь: `gh pr list --state open --label ship --json number,title,labels` минус
   `hold`, по возрастанию номера. Пусто → `ИТОГ: ГОТОВО (очередь пуста)` и стоп.
