...
{ repository(owner:"ivanarama", name:"onebase") {
    discussion(number:1158){ id title body createdAt author{login} category{name}
      comments(first:30){ nodes{ id author{login} createdAt body } } } } }' \
...
2. Кандидаты. Пройди список тредов и отбери те, где **последнее слово за
   человеком со стороны**: либо комментариев нет вовсе, либо автор последнего
   комментария не `ivanarama` и не `ivantit66` (тот же список «своих», что у
   `backlogsweep`). Отбрось треды, где в комментариях уже есть маркер
   `<!-- pp:discussion -->` и после него никто не писал: их ты уже разобрал.
