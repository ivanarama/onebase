...
5. Каждый оставшийся пункт обрабатывай crash-safe транзакцией. Его устойчивый
   ...
   Если claim для ключа нет, опубликуй его и перечитай состояние. Продолжать
   вправе только самый ранний доверенный claim этого ключа по `created_at`, затем
   `id`; конкурентные claims остаются диагностикой. Если уже есть доверенный
   completion
   ...
   раньше `created_at` winning claim, исключи PR и сравни полное тело с точным
   ...
   ```
   gh api --paginate "repos/ivanarama/onebase/issues?state=all&since=<claim-created-at>&per_page=100" \
...
6. Непосредственно перед точкой невозврата ещё раз перечитай labels, comments,
   выбранное заключение и claims, повтори точный поиск `pp:tail-source`. Только
   при открытом гейте и если exact-source issue всё ещё нет заводи заявку:
   ...
   запуску. После найденного или созданного issue опубликуй item-done с его
...
7. Только когда каждый неснятый пункт имеет доверенный item-done (созданный
   ...
   **Хвост разобран.** Заведено: #<a> (<коротко>), #<b> (<коротко>).
   ...
   Общий маркер обязателен, но защиту от дублей обеспечивают per-item claim,
   exact source и item-done: падение между `gh issue create` и этим комментарием
   больше не создаёт второй issue, а параллельный worker проигрывает claim.
