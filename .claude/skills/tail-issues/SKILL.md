...
2. Очередь: PR, влитые за последние 14 дней. Окно задаётся **в самом запросе**:
   ...
   **#1261**, который его вводит:
   ...
   для ограниченного legacy-drain: PR, влитые строго раньше неё, появились до
   `Reviewed-SHA`/`Outcome-Label`/`pp:head-reviewed` и законно не имеют
   committed-пары. Для такого PR без каноничной пары выбери **последнее**
   доверенное legacy-заключение `ivanarama` с точным отдельным tail-маркером
   `<!-- pp:review pp:tail=N -->`. Если после него есть доверенная отдельная
   строка `pp:review-again`, legacy-аудит отменён и fallback запрещён. PR,
   влитый в момент границы или позже, без каноничной пары всегда отбрасывается:
   legacy-fallback для нового протокола недопустим.
