...
аргументом `<N>` та же проверка обязательна; номер не обходит гейт.
...
последующий комментарий человека, одна `decision:N`, иначе `pp:recommend=N`.
...
только плановые/документирующие изменения, восстанови недостающий handoff и
заверши `ИТОГ: УЖЕ СДЕЛАНО`.
