...
2. Кандидаты: `gh issue list --state open --limit 100 --json number,title,labels`.
   ...
   `<!-- pp:triage -->` от `ivanarama` точной отдельной строкой. Комментарии для
   этого получай пагинированным REST; чужое или встроенное в текст упоминание не
   блокирует triage.
...
4. Оставь комментарий-разбор (в конце — обязательный маркер):
   ...
   **Триаж.**
   ...
   Маркер обязан быть точной отдельной строкой. Если из-за параллельного запуска
   появилось несколько доверенных комментариев `ivanarama` с такой строкой,
   каноничен самый ранний по GitHub `created_at`, затем по числовому `id`; FIX
   всегда читает именно его. После POST перечитай все комментарии: если
   собственный возвращённый id не каноничен, не ставь маршрутные метки и заверши
   этот item как проигравший гонку. Более поздний triage не заменяет план
   молча — для изменения плана редактируется каноничный комментарий, а его
