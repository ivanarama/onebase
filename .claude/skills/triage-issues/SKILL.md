...
ни того, что ему поверили, ни того, что делать сегодня. Проверка «внешняя заявка
без ответа» в `tools/backlogsweep` эту дырку уже показывает — отвечать нужно,
а не только считать молчание.

**Когда отвечать:** поставил `ready-fix` **и** автор заявки не `ivanarama` и не
`ivantit66` (тот же список «своих», что у `backlogsweep`). Ответ — **отдельный**
комментарий после разбора, не строка внутри него: разбор читает машина, ответ
читает человек. Маркер `<!-- pp:reply -->`; он же признак, что отвечать второй
раз не надо.
...
по коду. / Разобрали: воспроизвести не удалось — <чего не хватает>.
