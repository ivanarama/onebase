...
7. Заключение — комментарий в PR. Кандидат в заключения содержит отдельную
   ...
   нет, выбери самый ранний подходящий orphan текущей эпохи по `created_at`,
   затем `id`; опубликуй claim, если его ещё нет, и заново прочитай события.
   Поставить/подтвердить ожидаемую метку может только orphan, чей claim оказался
   самым ранним валидным claim эпохи; остальные не могут подменить Outcome-Label.
   Для выбранного orphan-комментария затем
