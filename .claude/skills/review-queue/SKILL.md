...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   **перед** обычной очередью и сортируй их по номеру. Если такой кандидат есть,
   ...
   этого запуска, остальные слоты не заполняй. Это single-flight-барьер: пока
   MERGE не вольёт владельца барьера, нельзя заранее ревьюить следующий PR,
   потому что первый merge снова изменит `main` и обесценит второй аудит. Только
   когда интеграционных кандидатов нет, применяй обычный
