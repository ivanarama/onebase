...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   `ship` тоже не является работой REVIEW, но есть одно точное исключение:
   `ship` сохранён MERGE после доказанного base-sync, текущий HEAD равен `to`
   валидного `pp:base-sync-done`, а committed-пары для этого HEAD ещё нет. Для
   ...
   Кандидатов с незавершённым доказанным base-sync ставь **перед** обычной
   очередью и сортируй их по номеру: человек уже разрешил мерж, осталось
   проверить только интеграцию с новым `main`. После них применяй обычный
   `(review-depth ASC, number ASC)`. Обычный `ship` без валидной carry-цепочки
   пропускай и не меняй: это либо готовый MERGE, либо безопасно закрытый гейт.
   ...
   **Не сортируй очередь только по номеру PR:** тогда старый PR после каждого
   ...
   меткам; единственное исключение для `ship` — валидная незавершённая base-sync
   carry-цепочка точного текущего HEAD. Итоговые метки разбери по таблице ниже.
...
7. Заключение — комментарий в PR. Кандидат в заключения содержит отдельную
   ...
   обычный `ship` без валидной carry-цепочки либо более новый override запрещают
   ...
   точного `to` валидной незавершённой carry-цепочки. Для обычного заключения
   `changes-requested` и
   ...
   **не удаляй общую метку**: у GitHub-метки нет владельца, и DELETE может снять
   ...
   Для интеграционного REVIEW с сохранённым `ship` после подтверждённого
