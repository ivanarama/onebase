...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   **перед** обычной
   ...
   или доказанного legacy re-ship
   ...
   **Не удаляй её перед аудитом** — у общей метки нет владельца, и DELETE может
   ...
   | есть `ship`, текущий HEAD ещё без committed-пары и валиден legacy re-ship после его anchor | интеграционное REVIEW; поставить перед обычной очередью |
