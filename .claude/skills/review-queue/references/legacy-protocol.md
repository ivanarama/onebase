...
5. **Хвост: что делать с неблокирующим.** Всё, что осталось за вычетом
   ...
   **с отступом**; одиночный прочерк `—` вместо списка вместе с `pp:tail=0`.
   ...
   `pp:tail` и на стороне TAIL останавливает разбор всего хвоста
   (`НУЖЕН ЧЕЛОВЕК`). Не публикуй заключение, пока раздел не соответствует этой
   грамматике: правило без проверки уже один раз пережило собственное нарушение
   (заключение PR #1200).
