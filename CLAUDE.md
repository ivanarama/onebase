...
- **Конвейер сопровождения — метки, а не договорённости.** Заявки и PR ведутся
  ...
  **починка — не решение, новая возможность — решение.**
  ...
  `ship`, если после аудита был push или `pp:review-again`; timeline обязана
  доказывать, что человек поставил `ship` после этого completion. ID комментариев
  ...
  итоговая метка → pp:head-reviewed` восстанавливается по `Reviewed-SHA`; только
  самый ранний claim эпохи вправе менять outcome-метку. Committed-маркер хранит
  ...
  FIX перепроверяет `ship` перед каждым внешним изменением и, увидев его, ветку
  больше не меняет даже при старой `changes-requested`. Узкое окно, где push уже
  ...
  - **Неблокирующие находки ревью не пропадают.** Ревью находит больше, чем
    ...
    для PR, влитых строго до мержа #1261, действует ограниченный drain последнего
    доверенного legacy-заключения; после этой границы без committed-пары TAIL
    ничего не принимает. Та же грамматика, что `approved` / `approved + decision:N`.
