...
- **Конвейер сопровождения — метки, а не договорённости.** Заявки и PR ведутся
  ...
  **починка — не решение, новая возможность — решение.**
  ...
  к человеку). Гейт автоматики один: `ship` на **PR**, ставит только человек;
  пастух PR без `ship` не вливает и не трогает вовсе. **В репозитории этого
  гейта нет:** защита ветки ревью не требует (`required_pull_request_reviews:
  null`), поэтому ручной мёрж проходит мимо метки — и уже проходил (#1193).
  Держит защита только обязательные проверки CI, зато для всех, включая
  владельца. `hold` останавливает
