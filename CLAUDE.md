...
- **Конвейер сопровождения — метки, а не договорённости.** Заявки и PR ведутся
  ...
  **починка — не решение, новая возможность — решение.**
  ...
  без `ship` не вливается автоматикой; ограничение «не трогать» относится к
  MERGE, тогда как REVIEW и FIX готовят PR до человеческого одобрения. На
  уровне репозитория этого гейта нет: защита ветки не требует ревью
  (`required_pull_request_reviews: null`), поэтому ручной мёрж может пройти
  мимо метки — и уже проходил (#1193). Обязательные проверки CI защита держит
  для всех, включая владельца. Исключения для
