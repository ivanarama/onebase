...
- **Конвейер сопровождения — метки, а не договорённости.** Заявки и PR ведутся
  ...
  **починка — не решение, новая возможность — решение.**
  ...
  - **Обсуждения — вход в проект мимо заявок, и у него нет меток.** Переключать
    там нечего, поэтому единственная сверка — этап `/discussions-watch` раз в
    сутки: тред, где последнее слово за человеком со стороны, получает ответ,
    а просьба — номер обычной заявки, которую дальше разбирает триаж на общих
    основаниях. Отвечать разрешено только тем, что проверено прогоном:
    обсуждения читают те, кто ещё выбирает платформу, и «так работает» без
    проверки обесценивает все остальные ответы разом. Этап ходит под
    провайдером `claude-discussions` с урезанными правами — писать в
    обсуждение может любой пользователь GitHub, это самый недоверенный текст в
    репозитории.
