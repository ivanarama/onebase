...
- **Гейты, которые нельзя обойти** (план 115): ветка `main` защищена, обязательны
  `build`, `lint`, `postgres-integration`, `vuln`, и включён `strict` — перед
  мержем в ветку надо влить свежий `main` и дождаться перепрогона. Аварийное
  снятие — `gh api -X DELETE …/branches/main/protection`, возврат —
  `-X PUT … --input .github/branch-protection.json`. Просадка производительности
  больше 25% валит джоб `bench` (`tools/benchgate`).
