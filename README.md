...
3. Двойной клик на **`Запустить.vbs`** — откроется лаунчер

Что в архиве:
```
onebase-gui.exe   ← лаунчер с нативным окном (двойной клик)
onebase.exe       ← консольная версия (для терминала)
Запустить.vbs     ← умный запускатор, выбирает нужный exe
