...
Ниже — то, что нужно чаще всего. Полный список (169 функций, 161 методов
...
- [`docs/dsl-reference.md`](docs/dsl-reference.md) — справочник встроенного языка: 169 функций, 161 методов, язык запросов. Сам файл генерируется из платформы (`onebase langref`) и сверяется с ней тестом — точные числа всегда в нём.
