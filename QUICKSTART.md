...
Откроется лаунчер информационных баз в нативном окне WebView2.
...
с изолированным профилем в нативном окне, если оно доступно, иначе во внешнем
Chromium-браузере. Через меню рядом можно явно выбрать **«Окно браузера
(Edge/Chrome)»**. Вход в новом профиле не заденет остальные окна.

### Шаг 4. Редактировать конфигурацию
...
```
Go 1.26.6      → https://go.dev/dl/                     (только для сборки из исходников)
PostgreSQL 14+ → https://www.postgresql.org/download/  (опционально: высокая параллельная нагрузка и серверная эксплуатация)
```
...
Ниже — то, что нужно чаще всего. Полный список (155 функций, 150 методов
