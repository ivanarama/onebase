...
// а показывает его форма и плитка списка.
//
// Идемпотентность: позиции с заполненным Фото пропускаются, а плашки
// сохраняются, только если есть кому их назначить. Это важнее, чем кажется:
// картинки из СохранитьКартинку помечены dsl-managed и НЕ удаляются сборкой
// мусора (`onebase gc-blobs` не знает, куда прикладной код положил UUID),
// поэтому лишние сохранения останутся в хранилище навсегда.
...
Процедура Выполнить()
  ...
  // остальные три dsl-managed блоба не должны навсегда осесть в хранилище.
  // Внутри категории UUID общий для всех позиций этого запуска.
  ФотоДерево = "";
  ФотоКрепёж = "";
  ФотоФурнитура = "";
  ФотоИнструмент = "";
  ...
  Для Каждого Стр Из БезФото Цикл
    Кат = КатегорияПоНаименованию(Стр.Наименование);
    Если Кат = "дерево" Тогда
      Если ЗначениеЗаполнено(ФотоДерево) = Ложь Тогда
        ФотоДерево = СохранитьКартинку(ПлашкаДерево(), "image/png");
      КонецЕсли;
      УУИД = ФотоДерево;
    ИначеЕсли Кат = "крепёж" Тогда
      Если ЗначениеЗаполнено(ФотоКрепёж) = Ложь Тогда
        ФотоКрепёж = СохранитьКартинку(ПлашкаКрепёж(), "image/png");
      КонецЕсли;
      УУИД = ФотоКрепёж;
    ИначеЕсли Кат = "фурнитура" Тогда
      Если ЗначениеЗаполнено(ФотоФурнитура) = Ложь Тогда
        ФотоФурнитура = СохранитьКартинку(ПлашкаФурнитура(), "image/png");
      КонецЕсли;
      УУИД = ФотоФурнитура;
    Иначе
      Если ЗначениеЗаполнено(ФотоИнструмент) = Ложь Тогда
        ФотоИнструмент = СохранитьКартинку(ПлашкаИнструмент(), "image/png");
      КонецЕсли;
      УУИД = ФотоИнструмент;
    КонецЕсли;

    // Колонка «Ссылка» в результате запроса — это UUID-строка, а не ссылка с
