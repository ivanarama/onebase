...
1. **D2/D3:** PostgreSQL advisory locks для `БлокировкаДанных` и проведения в
   нескольких процессах. Это не мультисессионность, а корректность
   межпроцессной записи.
2. **E UX:** background export/polling/SSE для долгих Excel/PDF выгрузок.
   Runtime limits и truncation уже есть; нужен удобный пользовательский контур.
3. **Load validation:** k6 PostgreSQL прогон на демо/реальной конфигурации с
   проверкой p95/p99, pool saturation и runtime operation metrics.
4. **G future:** shared file storage, external pub-sub и scheduler leader
