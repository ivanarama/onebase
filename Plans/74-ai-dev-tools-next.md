...
## Выполнено во втором срезе

Дата реализации: 2026-06-24.

- Добавлен пакет `internal/configfmt`: детерминированный YAML formatter с порядком
  известных ключей OneBase, отступом 2 пробела, раскрытием flow-map/list и
  идемпотентностью.
- Добавлена команда `onebase fmt [path ...]`:
  - без аргументов форматирует каталог из `--project`;
  - `--check` только проверяет и возвращает ненулевой код при расхождениях;
  - директории разворачиваются рекурсивно, `.git/.codex/node_modules/vendor`
    пропускаются.
- Сохранение YAML через конфигуратор теперь проходит через тот же formatter:
  file-mode (`writeConfigFileRaw`, batch `saveConfigFiles`, роли) и database-mode
  (`configdb.SaveFile`/`SaveFiles`/`ApplyFiles`). `.os`, изображения и прочие
  не-YAML файлы не меняются.
- Покрыто unit-тестами `configfmt`, CLI, `configdb` и launcher save-paths.

Массовое переформатирование существующих `examples/` не включено в этот срез:
