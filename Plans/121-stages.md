...
Статус: 🟡 **Сделан базовый срез 2026-08-12** (PR #772), поверх редакции плана
до её ужесточения в PR #768. Вырос из разбора плана 85: см. «Отношение к
плану 85» ниже.
...
- `_stage_history` + `EnsureStageHistorySchema` (вызов встроен в `db.Migrate`
  ...
  `systemTables`; порядок записей внутри секунды держится на дробном `at`;
- обмен помечает запись внешней (`WithExternalStageWrite`), история хранит
  `source`;
- отчёт «где застряло» (`/ui/stages`) с учётом построчного доступа и маски ПДн,
  история переходов в карточке, схема маршрута на ECharts (серия `graph`);
- `onebase check` (предупреждения), `describe`, `impact`, `ai-guide`,
  сохранение блока конфигуратором, локали, `docs/features.md`, объявленный
  маршрут в `examples/callcenter`.

### Что из ужесточённой редакции (PR #768) ещё НЕ сделано

- **Сериализация `read → check → write → history`.** Сейчас проверка идёт на
  best-effort `GetByID` без блокировки строки: два конкурентных запроса могут
  оба увидеть один старый этап и оба пройти гейт. Нужны `WithTxScope` +
  `AdvisoryXactLock`/`FOR UPDATE` на PostgreSQL и CAS по `_version` на SQLite,
  плюс матрица на двух независимых подключениях.
- **`SyncAllPredefined`** (`storage/predefined.go`) пишет поля прямым
  `INSERT … ON CONFLICT DO UPDATE` мимо обеих точек — третий writer, инвариант
  этапа там не соблюдается.
- **Fail-closed чтение старого значения.** `upsert` считает новым объектом любую
  ошибку `GetByID`, а не только «строки нет»; для гейта это означает, что сбой
  чтения открывает проход.
- **Причинный порядок истории (`event_no`)** вместо опоры на время; отдельный
  узкий storage-writer для обмена со структурным provenance вместо контекстного
  признака; фиксация семантики raw restore.
