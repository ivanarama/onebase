...
Статус: 🟡 **Ядро сделано 2026-08-12** (PR #772): декларация, сериализованный
гейт в обеих обычных точках записи, причинная история, доверенные writer-ы
миграции и репликации, отчёт и схема. Остаток ужесточённой редакции (PR #768) —
пакетная блокировка обмена, tombstone/resurrection, read-consistent архив,
авторизованный loader истории — перечислен ниже. Вырос из разбора плана 85:
см. «Отношение к плану 85».

### Что уже работает

- декларация `stages` у сущности + валидация (`metadata`), без нового вида
  объекта метаданных; пустое значение этапа трактуется как начало маршрута;
- **сериализованный цикл записи** для сущности с этапами: `WithTxScope` +
  advisory lock на PostgreSQL (`FOR UPDATE` не блокирует отсутствующую строку, а
  создание — такой же переход) и CAS по `_version` на SQLite; `SQLITE_BUSY`
  нормализуется в конфликт. Матрица гонит две записи из ДВУХ независимых
  подключений к одной базе: один SQLite-handle сериализовал бы всё сам и показал
  бы зелёное на дыре;
- гейт переходов в **обеих** обычных точках записи storage (`crud.go:upsert` и
  `optimistic_lock.go:UpsertVersioned`) с матричными тестами на всех четырёх
  прикладных путях — и на создании, и на правке существующего объекта;
  устаревшая ревизия остаётся конфликтом версий и решается ДО проверки перехода;
  сбой чтения старого значения не притворяется созданием;
- **причинный порядок истории**: `event_no`, назначаемый под той же блокировкой,
  с уникальным индексом `(entity_name, field_id, record_id, event_no)`; время
  осталось только для показа. Идентичность реквизита — устойчивый `Field.ID`
  (или имя в нижнем регистре, если `id` не объявлен), поэтому переименование не
  отвязывает историю;
- нарушение, пропущенное режимом `warn`, пишется в историю помеченным
  (`violation`), а внешнее предупреждение уходит в лог только после коммита;
- **третий writer** — синхронизация предопределённых: весь sync сущности с
  этапами идёт одной транзакцией под блокировкой (UUID раздаются до прохода по
  элементам, поэтому блокировка внутри элемента опоздала бы); явное значение
  обязано быть объявленным этапом и пишет одно событие `source=migration` с
  происхождением `["migration", сущность, имя]`; живой этап миграция не
  откатывает;
- **узкий writer репликации** `ApplyReplicatedEntity` вместо флага у обычной
  записи: обход списка переходов доступен только приёмке пакета, вызывается
  после решения «побеждает входящий» (DSL-хук конфликта остаётся обычным
  локальным writer-ом), происхождение — `["exchange", план, узел, сообщение]`,
  актор не подставляется;
- `_stage_history` + `EnsureStageHistorySchema` (вызов встроен в `db.Migrate`
  структурно, а не списком по командам, и продублирован во всех точках подъёма
  базы, включая импорт резервной копии); таблица включена в выгрузку
  `systemTables`;
- отчёт «где застряло» (`/ui/stages`) с учётом построчного доступа и маски ПДн:
  «последним» считается событие с максимальным `event_no`, а расхождение
  истории с текущим значением честно показывается как «время неизвестно», а не
  подставляет чужой срок; история переходов в карточке, схема маршрута на
  ECharts (серия `graph`);
- **авторизованное чтение истории**: право на сущность → построчный доступ к
  самой строке → редактирование значений по политике полей → и только потом
  обогащение (маскированное значение не форматируется как дата и не
  разыменовывается как ссылка). Тот же редактор применён к журналу регистрации,
  где до этого страница «История» показывала прежний и новый телефон целиком
  пользователю, которому в форме он показан замаскированным;
- `onebase check` (предупреждения), `describe`, `impact`, `ai-guide`,
  сохранение блока конфигуратором и знание о нём у линта YAML-ключей, локали,
  `docs/features.md`, объявленный маршрут в `examples/callcenter`.

### Что из ужесточённой редакции ещё НЕ сделано

- **Пакетная блокировка обмена**: сортированные advisory locks на все объекты
  пакета до цикла и один scope на весь пакет (сейчас сериализуется отдельная
  запись, а не пакет целиком); write-intent на SQLite в начале транзакции.
- **Tombstone/resurrection** обмена как отдельные узкие операции.
- **Read-consistent snapshot** в `ExportUniversal` (сейчас таблицы выгружаются
  последовательными autocommit-SELECT — архив может совместить объект до
  перехода с историей после него).
- **`WithProvisionalCreate`** как отдельная операция и отказ публичного
  `UpsertPreserveVersion` для staged-сущности: сейчас провизорное создание идёт
  общим staged-путём (insert-only + проверка числа строк), но собственного
  контракта у него нет.
- **Индекс поля-этапа** в миграции сущности с identity по `Field.ID`; тест на
  продолжение истории после переименования реквизита; `stages` в JSON Schema
  (`cli/schema.go`).
- **Схема маршрута в управляемой форме** (сейчас только автоформа) и
  идемпотентный инициализатор в `ui.js`.

### Три уточнения к замыслу, найденные при реализации

1. **Точек записи действительно две** — план это предсказал, и предсказание
   подтвердилось буквально: гейт только в `upsert` пропускал бы все правки из
   формы, REST и DSL.
2. **Пустой этап = начало маршрута.** Считать пустое значение отдельным
   состоянием нельзя: законный прикладной путь «реквизит не заполнен → отправили
   на согласование» (`examples/callcenter/src/Согласование.module.os`) оказался
   бы нарушением. Разрешать из пустого что угодно — оставить ровно ту дыру, ради
   которой гейт написан.
3. **Проведение — не создание.** DSL-проведение дописывает реквизиты после
   `ОбработкаПроведения` через `UpsertPreserveVersion`; если считать эту запись
   созданием, документ на не-начальном этапе перестаёт проводиться.
