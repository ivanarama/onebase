...
- `_stage_history` + `EnsureStageHistorySchema` (вызов встроен в `db.Migrate`
  ...
  `systemTables`;
- **пакетная блокировка обмена**: все объекты пакета блокируются до цикла и в
  порядке ключа, а не порядке объектов в пакете — иначе два встречных пакета с
  одними и теми же объектами встают друг против друга; tombstone не сочиняет
  переход, а воскрешение пишет ровно одно синтетическое событие;
- **read-consistent архив**: вся выгрузка (данные, системные таблицы, настройки,
  конфигурация и предпроверка вложений) читает базу на одном снимке —
  PostgreSQL `REPEATABLE READ READ ONLY`, SQLite — одна читающая транзакция.
  Без него объект попадал в архив до перехода, а его история — уже после;
- **индекс по полю-этапу** с идентичностью по `Field.ID`: отчёт группирует
  объекты по этапу, а переименование реквизита переносит индекс, а не оставляет
  рядом второй;
...
- схема маршрута общим блоком для автоформы и управляемой формы, с
  идемпотентным инициализатором;
- `onebase check` (предупреждения), `describe`, `impact`, `ai-guide`, `schema`,
...
- **Отдельный контракт провизорного создания** (`WithProvisionalCreate`) и отказ
  публичного `UpsertPreserveVersion` для staged-сущности. Сейчас провизорное
  создание идёт общим сериализованным путём (insert-only + проверка числа
  строк), и этого достаточно, чтобы не закоммитить строку без истории, но
  собственного имени у операции нет. Отказ публичного `UpsertPreserveVersion`
  требует отдельного решения: тем же вызовом DSL дописывает реквизиты после
  `ОбработкаПроведения`, и запрет сломал бы проведение.
- **DemoReset**: очистка возможных синтетических событий миграции перед
  импортом архива (сейчас restore полагается на общий clear/import).
