...
## Пробел, вскрытый конфигурацией (2026-08-18)

Публикация работает только со **вложениями** (`_attachments`), а поле типа
`image` хранится в блобах (`_blobs`, `SaveBlob`/`СохранитьКартинку`). Для CMS
это главный сценарий: картинка новости и фото товара лежат именно в поле
карточки, а не во вкладке вложений — то есть «публичная ссылка на файл» не
покрывает то, ради чего задумывалась.

Варианты (решать до слайса C6 плана 129):

1. Расширить `_public_files`: колонки `attachment_id` и `blob_id`, ровно одна
   заполнена; отдача выбирает источник. Появляется `ОпубликоватьКартинку(Ид)`.
2. Оставить как есть и в конфигурации хранить медиа вложениями. Неудобно:
   контент-менеджер грузит файл не в поле карточки, а в отдельную вкладку.

Рекомендация — вариант 1: пользовательский смысл у ссылки один, и делить его
по внутреннему устройству хранилища неправильно.
