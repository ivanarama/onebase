...
```
КэшСервисов.Очистить();              // весь кэш процесса
КэшСервисов.Очистить("Nortena");     // только один сервис
Всего = КэшСервисов.Размер();        // байт в кэше — для диагностики
```

Английские имена: `ServiceCache.Clear([name])`, `ServiceCache.Size()`.
...
7. **Single-flight на промах.** Пять одновременных запросов на «холодную»
   популярную страницу не должны запускать пять исполнений DSL (это ровно тот
   момент, когда сайт и падает). Берём `golang.org/x/sync/singleflight`
   (уже в `go.sum`, промоутим в прямые зависимости), ключ — ключ кэша.
   Ошибка исполнения не шарится между ждущими: каждый получает свой 500 —
   иначе один сбой отдаст одинаковую ошибку пятерым, и в логе будет одна
   запись вместо пяти.
...
| `internal/ui/dsl_service_cache.go` (новый) | Глобал `КэшСервисов`/`ServiceCache` (`Очистить`/`Clear`, `Размер`/`Size`) |
| `internal/dslvars/dslvars.go` | Инжекция глобала в общую карту, чтобы он был доступен и в регламентных заданиях (сброс кэша по расписанию), и в UI, и в сервисах |
