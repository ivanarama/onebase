...
4. **Хранение — в памяти процесса, LRU по суммарному размеру.** Лимит по
   умолчанию 64 МиБ на весь процесс (не на сервис). Ключ `app.yaml`
   (`http_cache_max_bytes`) в первой реализации НЕ сделан — лимит пока
   константа; настройка появится по первому реальному запросу.
   БД как хранилище не берём: кэш в БД
