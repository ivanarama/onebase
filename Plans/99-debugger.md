...
- **вычисление идёт без точек останова и шагов только в своём DSL-запуске**:
  `evalDebugExpr` временно отключает hook в per-run `execCtx`, поэтому условие
  вида `Удвоить(Сч) > 6` не входит в свою же строку, а параллельные запуски
  продолжают видеть собственные точки;
