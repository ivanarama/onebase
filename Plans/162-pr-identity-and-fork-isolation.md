...
Действующий гейт из #1427 остаётся границей полномочий FIX:
`headRepoFullName == "ivanarama/onebase"` обязателен для автоматической
доработки кода, потому что она меняет head-ref. Это не противоречит пункту 6:
CLEAN fork сливается REST API базового репозитория и не получает запись в
репозиторий автора. Передача BEHIND/DIRTY fork человеку также меняет только
состояние PR в базовом репозитории и не является записью в fork.
...
Эксплуатационное следствие включения `minimum_version=2`: каждый остающийся
открытым same-repo PR с v1 proof должен получить bridge и новый человеческий
`ship`, даже если до cutover он уже был отревьюирован и имел `ship`. Поэтому
минимальную версию повышают только после разгона текущей очереди мержей, чтобы
не отправлять всю накопленную очередь на повторную ручную авторизацию разом.
...
1. Выпустить PromptPilot с v2, не повышая пока минимальную версию onebase.
2. Разогнать текущую очередь мержей и только затем поднять `minimum_version`.
3. Обновить канонические skills, адаптеры и maintenance-документацию.
4. Наблюдать одно релизное окно; проверить открытые v1 proofs и recovery.
5. Убрать v1 как merge-authority, оставив понятную диагностику.
