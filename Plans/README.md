...
9. Стратегические, каждый отдельным проектом: **84** (enterprise-auth),
   **60B** (marketplace). **85** (движок бизнес-процессов) 🧊 заморожен
   2026-08-12 — вместо него **119** (этапы: гейт переходов, история,
...
| 117 | [117-standard-code-and-base-prefix.md](117-standard-code-and-base-prefix.md) | Стандартный «Код» справочников, единая точка автонумерации, префикс базы для кодов и номеров | ⬜ Проект 2026-08-08 (issue #658), не начато. Этап 117A (фиксы конвертера 1С) берётся отдельно |
| 118 | [118-list-detail-panel.md](118-list-detail-panel.md) | Боковая панель деталей активной записи списка (+ закладки внутри панели) | 🟡 Проект 2026-08-09 (issue #670). 118A сделан; 118B–118D не начаты, самодостаточен 118B. Вторую половину #670 (событие `ПриАктивизацииСтроки` для ТЧ) план не покрывает — закрыта отдельно как дефект |
| 119 | [119-inforeg-write-api.md](119-inforeg-write-api.md) | Программная запись регистров сведений из DSL (менеджер записи, набор записей) | ⬜ Проект 2026-08-11 (issue #743), не начато. Этап 119A самодостаточен |
