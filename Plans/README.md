...
| 122 | [122-startup-smoke-and-platform-gates.md](122-startup-smoke-and-platform-gates.md) | Гейты запуска: smoke-запуск собранного бинаря из общего каталога, Windows-раннер в CI, гейт публикации релиза, типизированная «невозможность самообновления» | 🟡 A (smoke-запуск собранного бинаря из общего каталога, матрица ubuntu×windows и стабильный required-check `smoke`) сделан; C (гейт публикации релиза) закрыт PR #782, исправление дефекта #874 — отдельный PR #922. B (windows-раннер тестов) упирается в E: первый прогон на windows-latest вскрыл шесть тестов, зависящих от среды. Не начаты B, D, E, F. До этапа A CI не запускал собранный лаунчер; до этапа B windows-only тесты по-прежнему не исполняются |
