...
| 117 | [117-standard-code-and-base-prefix.md](117-standard-code-and-base-prefix.md) | Стандартный «Код» справочников, единая точка автонумерации, префикс базы для кодов и номеров | ✅ Реализовано 2026-08-12/13 (117A–117F): конвертер, синтез «Кода», единая точка нумерации (`GenerateNumber`) и маски даты, префикс базы, `numerator.unique`, редактор нумератора в конфигураторе, `onebase renumber`. Остаток вынесен заявками: #868 (ID синтезированного «Номера»), #869 (нумерация справочников вне UI-пути), #871 (сброс префикса при restore через лаунчер), #872 (конвертер не переносит автонумерацию), #873 (`check --lint` против `fields[].id`) |
...
| 122 | [122-startup-smoke-and-platform-gates.md](122-startup-smoke-and-platform-gates.md) | Гейты запуска: smoke-запуск собранного бинаря из общего каталога, Windows-раннер в CI, гейт публикации релиза, типизированная «невозможность самообновления» | 🟡 Этап C (гейт публикации релиза) закрыт PR #782 в день написания плана — `release.yml` ждёт зелёный CI на том же коммите, права сужены; у гейта известный дефект #874. Остальное (A — smoke собранного бинаря, B — Windows-раннер, D — типизированная «невозможность самообновления», E, F) не начато — заявка #876. 4–6 дней, минимальный срез 1.5–2 (этапы A и C). CI ни разу не запускает лаунчер, все джобы `ubuntu-latest` → 5 windows-only тестовых файлов не исполняет ничто, а `release.yml` публикует независимо от зелёности CI |
| 123 | [123-run-job-from-dsl.md](123-run-job-from-dsl.md) | Запуск регламентного задания из DSL: `РегламентныеЗадания.Запустить` → идентификатор прогона, `Прогон(Ид)` → статус | ⬜ Проект 2026-08-13 (issue #742), реализация — внешний контрибьютор (@rsyuzyov). Контракт: «запустить сейчас» без параллелизма, ожидания и параметров; пул исполнителей — отдельная заявка |
