...
| 124 | [124-document-write-paths.md](124-document-write-paths.md) | Одна точка записи документа: свести три реализации (Save, docWriter, postDocument) | ⬜ Проект 2026-08-18 (issue #962, находка Н3). Рекомендация: сперва паритет отказов тестом, потом общий шов; полное сведение — направление, не задача |
| 131 | [131-html-table-builder.md](131-html-table-builder.md) | Строитель HTML-таблиц: значения в разметку только экранированными, по построению | ⬜ Проект 2026-08-18 (issue #988, находка А4). Сегодня 79 ручных `html.EscapeString` и 7 конверсий `template.HTML` с `//nolint:gosec`; ни одно место не сломано, но гарантия держится на дисциплине. Ключевое — не пакет, а правило линтера: строитель, которым можно не пользоваться, это стиль, а не гарантия. `pageRaw` (allowlist для пользовательского HTML) и `icons.go` вне периметра осознанно |
