...
//
// Подпись проверяется, если в сборку вшит открытый ключ. Её отсутствие —
// отказ в сборках с RequireSignature, а официальные релизы собираются с ним
// (#1195). Неверная подпись — отказ всегда, в любом режиме.
...
//   - подписи нет в релизе — отказ в жёстком режиме, а официальные сборки
//     выпускаются жёсткими (#1195); в мягком — строка в журнал, чтобы
//     отсутствие подписи не было тихим;
