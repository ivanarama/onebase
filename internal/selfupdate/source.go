...
// VersionRecognized сообщает, что версию вообще можно сопоставить с выпусками:
// она разобрана как build-N или vX.Y.Z.
//
// Нераспознанная версия («build-793fix», «dev-<sha>», пустая) — не «свежая» и
// не «устаревшая», а несравнимая: Newer отказывает по ней всегда, поэтому
// обновление такому бинарю не предложат никогда, каким бы старым он ни был.
// Интерфейс обязан говорить именно это, а не «установлена актуальная версия» —
// последнее прямо неверно и снимает вопрос, которого никто не задал (#1065).
func VersionRecognized(v string) bool { return parseVersion(v).kind != kindUnknown }
