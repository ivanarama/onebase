...
// KnownVersionScheme сообщает, что версию вообще можно сравнить с выпусками.
// Ложь для dev-сборок и для нестандартных ярлыков вроде `build-793fix`: Newer
// на таких отказывает всегда, то есть обновление не будет предложено никогда.
// Интерфейс обязан сказать это прямо — иначе он показывает «установлена
// актуальная версия» бинарю, который просто выпал из сравнения.
func KnownVersionScheme(s string) bool { return parseVersion(s).kind != kindUnknown }
