...
Допустимые kind элементов (других НЕТ): ПолеВвода, Надпись, Кнопка, Таблица, ГруппаФормы, Страница, СтраницыФормы, Флажок, Переключатель, ПолеСписка, ПолеДаты, ПолеФормы, ТабличнаяЧасть, Колонка, КоманднаяПанель, ПолеКартинки, КнопкаКП. Не выдумывай виды: поле-картинка (тип image) — это обычный kind: ПолеВвода (виджет загрузки рисуется по типу поля); «ПолеФайла»/«ПолеИзображения» не существуют.
