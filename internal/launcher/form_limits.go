...
// parseFormLimited разбирает форму. Возвращает ошибку — вызывающий обязан
// прекратить обработку, а не работать с пустыми FormValue.
//
// Обработчик по-прежнему ставит СВОЙ предел строкой
// `r.Body = http.MaxBytesReader(w, r.Body, <предел>)` перед разбором: правильное
// значение зависит от того, что он принимает — 8 МиБ для исходника модуля,
// десятки мегабайт для PDF, гигабайты для архива резервной копии. Единого
// значения тут нет.
//
// Но «нет единого значения» не означает «можно без значения». Раньше предел
// целиком лежал на обработчике, а страховкой считался gosec G120 — и страховка
// не работала (issue #630): G120 распознаёт только присваивание r.Body в теле
// САМОЙ функции, а разбор идёт через parseBoundedForm, у которой нет
// http.ResponseWriter. В итоге четыре обработчика читали тело без предела,
// причём два из них — cfgLoginSubmit и cfg2FASubmit — доступны БЕЗ
// аутентификации, и линтер молчал.
//
// Поэтому здесь стоит консервативное умолчание maxFormBody. Обработчики со
// своим пределом от него не страдают: ни один путь с большим пределом (архивы,
// PDF, логотип, предпросмотр) через parseFormLimited не проходит — проверено, —
// а меньший предел вызывающего срабатывает раньше внешнего. Забывчивость теперь
// даёт 413 вместо чтения тела без ограничения.
