...
//
// Предел тела ставит САМ обработчик, строкой
// `r.Body = http.MaxBytesReader(w, r.Body, <предел>)` перед первым обращением к
// форме. Это не забывчивость, а осознанный выбор: правильный предел зависит от
// того, что обработчик принимает — 64 КиБ для формы входа, 8 МиБ для исходника
// модуля, 64 ГиБ для архива резервной копии. Единого значения тут нет, и
// прятать его в помощник значит делать вид, что оно есть.
//
// За тем, что предел не забыли, следит gosec (G120): он распознаёт только
// присваивание r.Body в теле самой функции — через помощник не видит. Здесь это
// свойство работает на нас: линтер ловит ровно тот случай, когда обработчик
// добавили, а лимит не поставили.
func parseFormLimited(w http.ResponseWriter, r *http.Request) error {
	_ = w // предел ставит вызывающий; w оставлен для единообразия сигнатуры
