...
func (h *handler) configuratorSaveEnum(w http.ResponseWriter, r *http.Request) {
	...
	saveErr := h.guardConfigLoadable(r.Context(), b, func() error {
		if b.ConfigSource == "database" {
			...
			return cfgUpsert(r.Context(), db, path, out)
		}
		...
		return os.WriteFile(targetFile, out, fsmode.File) //nolint:gosec // G703: имя объекта проверено validObjectName, а сам путь получен обходом каталога проекта
	})
