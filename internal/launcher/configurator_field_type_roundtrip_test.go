...
`)
	...
	// «картинка» не было ни в отрисованной строке, ни в списке новой строки
	// (его рисует cfgAddField), то есть поле-картинку конфигуратор умел только
	// сломать, но не создать. Что пункт есть и у новой строки, сторожит
	// TestCfgTypeSets_MatchScriptOptions — иначе здесь подставлялось бы
	// значение, которого интерфейс не даёт, и тест врал бы ровно так же, как
	// врал прежний.
