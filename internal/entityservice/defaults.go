...
//
// ЕДИНСТВЕННАЯ реализация на все пути создания объекта: форма
// (ui.Server.form), DSL (Документы.X.Создать()) и REST (createObject,
// createObjectV2). Отдельная реализация «для формы» дала бы четвёртое
// расхождение к трём путям записи документа — тот же класс дефекта, что #366
// (DSL не звал OnUnpost) и разбор недели 16.08 (#962).
//
...
// достаточно объявить `default: сейчас` — тогда значение появится во всех
// трёх путях создания.
