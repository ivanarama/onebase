...
// при открытии и ui.Server.submit при записи управляемой формы), DSL
// (Документы.X.Создать()), REST (createObject, createObjectV2) и действие
// ИИ-чата (ui.Server.aiActionRun).
...
//
// У управляемой формы путей два, а не один, и это не дублирование: GET считает
// значения для показа, но форма рисует только размещённые элементы, поэтому
// неразмещённый реквизит до POST не доезжает и на записи считается заново
// (#1189). Автоформа вызывает NewObject только при открытии. Полный разбор —
// ui.Server.applyDefaultsToUnsubmittedFields.
//
...
type NewObjectResult struct {
	...
	Filled []string
	// DSLError != "" — хук ПриСозданииНового вернул ошибку. При открытии формы
	// пользователь видит причину; вызывающие пути записи считают её отказом.
	DSLError    string
	DSLMessages []string
}
