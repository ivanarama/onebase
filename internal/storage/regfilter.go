...
// Присутствие имени измерения в Dims/DimValues и есть отбор: пустая строка —
// такое же точное значение, как любое другое (у строкового измерения это
// законный ключ), поэтому она даёт условие, а не пропуск. «Отбора нет»
// выражается отсутствием ключа в карте; вызывающая сторона обязана не класть
// туда значения, которых пользователь не задавал.
...
func dimWhereClause(d Dialect, dims []metadata.Field, f RegFilter, startIdx int, includeFrom, includeTo bool) (string, []any, error) {
	var conds []string
	var args []any
	idx := startIdx
	...
	for _, fld := range dims {
		...
		arg, err := normalizeRegField(d, fld, val)
		if err != nil {
			return "", nil, fmt.Errorf("register filter dimension %s: %w", fld.Name, err)
		}
