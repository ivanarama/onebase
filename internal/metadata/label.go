...
func RowLabel(row map[string]any, e *Entity) string {
	...
	if skipEmpty {
		// Явный список заменяет автоматическое правило целиком. В частности,
		// документ с пустыми кандидатами не должен внезапно представиться
		// Номером/Датой, которых автор в presentation не указывал.
