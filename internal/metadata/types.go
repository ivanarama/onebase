...
type Field struct {
	...
	Default string
	// Multiline — реквизит редактируется многострочным полем (`multiline: true`).
	// Признак ПРЕДСТАВЛЕНИЯ, а не хранения: тип остаётся string, в базе тот же
	// TEXT. Нужен там, где в строковом реквизите лежит абзац, а не значение:
	// памятка, инструкция, комментарий. Без него автоформа карточки и форма
	// записи регистра рисуют однострочный input, и такой текст нельзя ни
	// прочитать, ни отредактировать, не перематывая его стрелками.
	//
	// richtext здесь не подходит: он про РАЗМЕТКУ (Quill, HTML в базе), а тут
	// обычный текст, который показывается как есть.
	Multiline bool
