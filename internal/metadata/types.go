...
type Field struct {
	...
	//
	// Field — один тип для реквизита шапки, поля табличной части и измерения
	...
	// (Entity.Fields) и измерения/ресурсы/реквизиты регистров — накопления,
	// сведений и бухгалтерского (storage.*PredicateEntity). Поля табличных
	// частей туда не попадают вовсе: field_access их не адресует, маскировать
	// их платформа не умеет. Поэтому `pii` на поле ТЧ не молчаливый ноль, а
	// ошибка конфигурации — её выдаёт Validate. Маркер, который по умолчанию
	// не защищает, был бы просто комментарием, а комментарий в поле с телефоном
