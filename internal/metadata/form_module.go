...
type FormElement struct {
	...
	// ChoiceFilter — ОТБОР подбора: «реквизит выбираемого справочника» → «путь к
	// значению на форме». Аналог связей параметров выбора в 1С и настраивается
	// так же — без кода:
	//
	//	- kind: ПолеВвода
	//	  data_path: Объект.Договор
	//	  choice_filter: { Владелец: Объект.Контрагент }
	//
	// Договор выбирается только из договоров указанного контрагента; сменили
	// контрагента — список перестроился, а значение, выпавшее из отбора,
	// очистилось. Подчинённому справочнику (owner: у справочника) писать это не
	// нужно: отбор по владельцу платформа ставит сама. ChoiceFilter нужен там,
	// где отбирают НЕ по владельцу (договоры по организации) или где ссылок на
	// владельца в объекте несколько и выбрать надо конкретную.
	ChoiceFilter map[string]string `yaml:"choice_filter,omitempty"`
