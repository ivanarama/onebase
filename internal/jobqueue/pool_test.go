...
// где удобно.
//
// PostgreSQL-половина работает, только если задан TEST_DATABASE_URL, а он есть
// ровно в одном джобе CI — и пакеты тот джоб перечисляет поимённо. Строка
// `go test ./internal/jobqueue` в `postgres-integration` (.github/workflows/
// ci.yml) поэтому обязательна: без неё эти тесты вечно гоняются на SQLite с
// одним исполнителем, показывают зелёное и не проверяют главное.
