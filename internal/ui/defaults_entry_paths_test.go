...
// создания объекта (контракт internal/entityservice/defaults.go: форма —
// автоформа и управляемая форма, DSL, REST, действие ИИ-чата). Здесь
// проверяются форма, DSL, управляемая форма и действие ИИ-чата; REST-путь
...
// Путь «форма» на управляемой форме (её POST). Она рисует только размещённые
...
// Действие ИИ-чата — четвёртый путь создания (после формы, DSL и REST):
// он строил объект напрямую (runtime.NewObject + Save) и не видел ни
// дефолтов, ни хука. Заявка #1189 назвала это заодно, решение человека —
// чинить (вариант 2 разбора).
