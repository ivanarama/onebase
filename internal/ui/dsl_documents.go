...
// runtimeObject отдаёт запись, стоящую за объектом документа из кода.
//
// Через этот интерфейс fill принимает объект-основание. Его уже реализуют
// модуль документа и объект управляемой формы, а docWriter — нет: обработка
// (и тест уровня конфигурации) падала «ожидается ссылка или объект, получено
// *ui.docWriter» на обычном
//
//	Звонок = Документы.А_Звонок.Создать(); … Заявка.Заполнить(Звонок)
//
// Заполнение по ССЫЛКЕ тут не замена: у ссылки реквизиты через точку
// недоступны, и ОбработкаЗаполнения получает основание с пустыми полями.
func (w *docWriter) runtimeObject() *runtime.Object {
	if w == nil {
		return nil
	}
	return w.obj
