...
func runCheckWithOptions(cmd *cobra.Command, opts configcheck.Options) error {
	...
	if !res.OK {
		// errSilentExit вместо os.Exit(1): код возврата тот же, но команда
		// доходит до конца, и отложенная уборка отрабатывает штатно. Прямой
		// os.Exit пропускал ВСЕ defer в цепочке вызова, поэтому уборку
		// приходилось звать руками — а такой список легко разъезжается с
		// реальными defer. Заодно команда становится проверяемой: тест,
		// вызвавший её на сломанной конфигурации, раньше убивал процесс
		// прогона целиком (#988, А6).
