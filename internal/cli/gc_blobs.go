...
пропускаются — они могут быть ещё не привязаны к записи. Легаси-блобы без
известной даты создания также защищены и показываются отдельной строкой.

Legacy-картинки, сохранённые из DSL вызовом СохранитьКартинку без владельца,
не удаляются никогда: их UUID мог быть положен прикладным кодом в строковый
реквизит, константу или регистр — туда сборка мусора не смотрит. Owner-aware
вызов с третьим аргументом учитывается как обычное image-поле.
...
func runGcBlobs(cmd *cobra.Command, _ []string) error {
	...
	if st.ProtectedLegacy > 0 {
		outf("Защищено без даты:   %d (legacy, возраст неизвестен)\n", st.ProtectedLegacy)
	}
	// Отдельной строкой, иначе legacy-картинки из СохранитьКартинку попадали в счётчик
	// grace-окна и вывод уверял, что блоб «моложе 0s», хотя защищён он навсегда.
	if st.ProtectedDSL > 0 {
		outf("Защищено как DSL:     %d (legacy без владельца — sweep их не трогает)\n", st.ProtectedDSL)
	}
	outf("Сирот:                %d\n", len(st.Orphans))
	if doDelete {
		outf("Удалено:             %d\n", st.Deleted)
