...
// Справка команды обязана называть все ключи: в headless-окружении она —
// единственный способ узнать, что вообще можно переключить.
//
// Раньше тест назывался «ТекстыОтказов_НазываютКлючИКоманду», хотя ни одного
// текста отказа не трогал — смотрел только справку. Сами отказы при этом годами
// звали в несуществующий экран «Система → Настройки», и зелёный тест с таким
// именем создавал впечатление, что их кто-то проверяет. Проверка настоящих
// текстов — в internal/launcher/guard_hint_path_test.go: она сверяет их с
// отрисованной страницей «Параметры базы», а не с ожиданием в самом тесте.
func TestСправкаSettings_НазываетКлючиИОписания(t *testing.T) {
