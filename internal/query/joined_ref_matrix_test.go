...
func TestJoinedRefFieldMatrix(t *testing.T) {
	dbtest.ForEachDialect(t, func(t *testing.T, db *storage.DB) {
		...
		for _, c := range []struct{ name, q, want, wantErr string }{
			...
			{
				// Круг 5 #1385: производная таблица — свой SELECT-scope со своим
				// главным источником и своими присоединёнными источниками; их
				// ссылочные поля разрешаются так же, как на верхнем уровне.
				"присоединённый внутри производной таблицы",
				`ВЫБРАТЬ Под.Наименование ИЗ (ВЫБРАТЬ П.Наименование ИЗ Справочник.Базы КАК Б ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Планы КАК П ПО П.ГлавныйУзел = Б.Ссылка) КАК Под`,
				"План-1",
				"",
			},
			{
				// Навигация внутри производной таблицы: авто-JOIN обязан
				// остаться в скобках подзапроса, а не выйти наружу.
				"навигация у присоединённого внутри производной таблицы",
				`ВЫБРАТЬ Под.ИмяУзла ИЗ (ВЫБРАТЬ П.Наименование КАК ИмяПлана, П.ГлавныйУзел.Наименование КАК ИмяУзла ИЗ Справочник.Базы КАК Б ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Планы КАК П ПО П.ГлавныйУзел = Б.Ссылка) КАК Под`,
				"Главная",
				"",
			},
		} {
