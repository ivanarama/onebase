...
func TestRefColumns(t *testing.T) {
	cases := []struct {
		...
		{
			name: "явный JOIN сохраняет тип ссылочной колонки",
			src:  `ВЫБРАТЬ р.Ссылка ИЗ Документ.Приход КАК р ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Проект КАК п ПО р.Проект = п.Ссылка`,
			want: map[string]string{"id": "Приход"},
		},
		{
		...
		{
			// Разбор проекции отключает не ОБЪЕДИНИТЬ как таковой, а ВТОРОЙ
			// ВЫБРАТЬ в тексте — в том числе подзапросом в ГДЕ, где список
			// выборки внешнего запроса выглядит совершенно простым. Граница
			// закреплена тестом потому, что теперь о ней говорят вслух четыре
			// поставляемых текста (ai-guide, langref, dsl-reference, features).
			name: "подзапрос отключает разбор так же, как ОБЪЕДИНИТЬ",
			src:  `ВЫБРАТЬ Ссылка ИЗ Документ.Приход ГДЕ Проект В (ВЫБРАТЬ Ссылка ИЗ Справочник.Проект)`,
			want: nil,
		},
	}
	for _, c := range cases {
