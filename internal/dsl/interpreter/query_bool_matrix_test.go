...
func TestQueryBoolFalseIsFalsy(t *testing.T) {
	dbtest.ForEachDialect(t, func(t *testing.T, db *storage.DB) {
		...
		assert.Equal(t,
			"Активный:да/тип=Булево/заполнено=true/=Истина=true;"+
				"Выключенный:нет/тип=Булево/заполнено=false/=Истина=false;",
			runOnDB(t, db, ent, src))
	})
}

// Числовой ноль из запроса тоже обязан быть ложным. Приведение колонок к булеву
// типу этот случай не закрывает: агрегат — не поле, его тип неизвестен, а на
// SQLite КОЛИЧЕСТВО() приезжает как int64 и раньше проходило как «истина».
func TestQueryNumericZeroIsFalsy(t *testing.T) {
	dbtest.ForEachDialect(t, func(t *testing.T, db *storage.DB) {
		ctx := context.Background()
		ent := boolFlagCatalog()
		require.NoError(t, db.Migrate(ctx, []*metadata.Entity{ent}))
		require.NoError(t, db.Upsert(ctx, ent.Name, uuid.New(),
			map[string]any{"Наименование": "Активный", "Активен": true}, ent))

		const src = `Процедура Тест()
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ КОЛИЧЕСТВО(Наименование) КАК К ИЗ Справочник.ПрофилиИзвлечения ГДЕ Наименование = ""нет такого""";
			Стр = Запрос.Выполнить()[0];
			Ветка = "нет";
			Если Стр.К Тогда
				Ветка = "да";
			КонецЕсли;
			Возврат Ветка + "/заполнено=" + Строка(ЗначениеЗаполнено(Стр.К));
		КонецПроцедуры`

		assert.Equal(t, "нет/заполнено=false", runOnDB(t, db, ent, src))
	})
}
