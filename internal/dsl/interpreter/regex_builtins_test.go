...
func TestRegex_Replace(t *testing.T) {
	...
	// #997: форма «$имя» без скобок для русской группы молча давала пустую
	// строку — Expand читает имя через unicode.IsLetter, находит группу «год»,
	// которой в скомпилированном шаблоне нет (она переименована в псевдоним), и
	// подставляет пустоту.
	t.Run("русская группа без скобок", func(t *testing.T) {
		src := `Процедура Тест()
			Возврат Новый Регекс("(?P<год>\d{4})-(?P<месяц>\d{2})").Заменить("2026-08", "$месяц.$год");
		КонецПроцедуры`
		assert.Equal(t, "08.2026", evalRegex(t, src))
	})

	// Имя читается целиком, а не по первому совпадению: «$годы» — это ссылка на
	// группу «годы», а не на «год» с хвостом «ы».
	t.Run("одно имя — префикс другого", func(t *testing.T) {
		src := `Процедура Тест()
			Возврат Новый Регекс("(?P<год>\d{4}) (?P<годы>\S+)").Заменить("2026 весь", "[$годы|$год]");
		КонецПроцедуры`
		assert.Equal(t, "[весь|2026]", evalRegex(t, src))
	})

	// «$$» — экранированный доллар: имя за ним не читается, текст остаётся как есть.
	t.Run("экранированный доллар рядом с русской группой", func(t *testing.T) {
		src := `Процедура Тест()
			Возврат Новый Регекс("(?P<сумма>\d+)").Заменить("120", "$$$сумма");
		КонецПроцедуры`
		assert.Equal(t, "$120", evalRegex(t, src))
	})

	// Ссылка на несуществующую группу остаётся поведением Expand — пустая
	// строка, а не ошибка; проверяем, что перевод псевдонимов её не искажает.
	t.Run("ссылка на несуществующую группу", func(t *testing.T) {
		src := `Процедура Тест()
			Возврат "[" + Новый Регекс("(?P<год>\d{4})").Заменить("2026", "${месяц}") + "]";
		КонецПроцедуры`
		assert.Equal(t, "[]", evalRegex(t, src))
	})
}

// #997: псевдоним русской группы не должен затирать пользовательскую группу с
// таким же именем. Раньше «(?P<ob1>…)» и «(?P<год>…)» компилировались обе в
// «ob1» — RE2 дубликаты имён не отвергает, и в ГруппыПоИмени побеждала
// последняя.
func TestRegex_NamedGroupsAliasCollision(t *testing.T) {
	t.Run("ГруппыПоИмени", func(t *testing.T) {
		src := `Процедура Тест()
			М = Новый Регекс("(?P<ob1>[A-Z]+)-(?P<год>\d{4})").Найти("AB-2026");
			Возврат М.ГруппыПоИмени.Получить("ob1") + "/" + М.ГруппыПоИмени.Получить("год");
		КонецПроцедуры`
		assert.Equal(t, "AB/2026", evalRegex(t, src))
	})

	t.Run("Заменить", func(t *testing.T) {
		src := `Процедура Тест()
			Возврат Новый Регекс("(?P<ob1>[A-Z]+)-(?P<год>\d{4})").Заменить("AB-2026", "${год}:${ob1}");
		КонецПроцедуры`
		assert.Equal(t, "2026:AB", evalRegex(t, src))
	})
}
