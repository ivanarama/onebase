...
func (i *Interpreter) evalCall(c *ast.CallExpr, e *env) any {
	...
	case *ast.MemberExpr:
		...
		case string:
			// Строка методов не имеет, и раньше такой вызов молча возвращал
			// Неопределено — падало уже следующее обращение, сообщая не про ту
			// строку. Практически всегда это колонка «Ссылка» из результата
			// запроса: там лежит UUID, а не ссылка с менеджером.
			if refMethodOnString(method) {
				RaiseUserError(callee.Field.Literal + "() вызван у строки: колонка «Ссылка» " +
					"в результате запроса — это UUID, а не ссылка. Получите ссылку через " +
...
// имеет смысл только у Ref (Ссылка.ПолучитьОбъект() и соседи). Остальные вызовы
// у строки оставлены как были: неизвестный метод — не повод менять поведение
// работающих конфигураций, а эти три говорят о конкретной ошибке.
