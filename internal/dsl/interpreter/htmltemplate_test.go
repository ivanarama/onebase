...
func TestHTMLTemplate_Funcs(t *testing.T) {
	...
	// #999: number на SQLite хранится как TEXT, и значение из Запрос.Выполнить()
	// доходит до шаблона строкой — раньше «число» уводило такую строку в default
	// и молча возвращало пустоту.
	t.Run("число строкой из базы", func(t *testing.T) {
		src := `Процедура Тест()
			Д = Новый Структура;
			Д.Вставить("Сумма", "1234567.5");
			Возврат Новый ШаблонHTML("{{число .Сумма 2}}").Заполнить(Д);
	...
	t.Run("число запятой как разделителем", func(t *testing.T) {
		src := `Процедура Тест()
			Д = Новый Структура;
			Д.Вставить("Сумма", "12,5");
			Возврат Новый ШаблонHTML("{{число .Сумма 2}}").Заполнить(Д);
		КонецПроцедуры`
		assert.Equal(t, "12.50", evalTemplate(t, src))
	})

	// Правило то же, что у «дата»: не разобралось — отдаём как есть, а не пустоту.
	t.Run("число неразобранной строкой отдаётся как есть", func(t *testing.T) {
		src := `Процедура Тест()
			Д = Новый Структура;
			Д.Вставить("Сумма", "не число");
			Возврат Новый ШаблонHTML("{{число .Сумма 2}}").Заполнить(Д);
		КонецПроцедуры`
		assert.Equal(t, "не число", evalTemplate(t, src))
	})

	t.Run("число пустой строкой — пусто, а не ноль", func(t *testing.T) {
		src := `Процедура Тест()
			Д = Новый Структура;
			Д.Вставить("Сумма", "");
			Возврат "[" + Новый ШаблонHTML("{{число .Сумма 2}}").Заполнить(Д) + "]";
...
// #999: платформа регистронезависима, а имена функций шаблона были известны
// только в нижнем регистре — нормализация по дереву понижает лишь имена полей и
// до функций дотянуться не может (text/template проверяет их на этапе Parse).
// «{{Дата .Дата}}» падало с англоязычным «function "Дата" not defined».
func TestHTMLTemplate_FuncNamesAreCaseInsensitive(t *testing.T) {
	cases := map[string]string{
		"Дата":   `{{Дата .Дата ""02.01.2006""}}`,
		"ДАТА":   `{{ДАТА .Дата ""02.01.2006""}}`,
		"Date":   `{{Date .Дата ""02.01.2006""}}`,
		"DATE":   `{{DATE .Дата ""02.01.2006""}}`,
		"Число":  `{{Число .Сумма 2}}`,
		"ЧИСЛО":  `{{ЧИСЛО .Сумма 2}}`,
		"Number": `{{Number .Сумма 2}}`,
		"NUMBER": `{{NUMBER .Сумма 2}}`,
	}
	want := map[string]string{
		"Дата": "17.08.2026", "ДАТА": "17.08.2026", "Date": "17.08.2026", "DATE": "17.08.2026",
		"Число": "1 234.50", "ЧИСЛО": "1 234.50", "Number": "1 234.50", "NUMBER": "1 234.50",
	}
	for name, expr := range cases {
		t.Run(name, func(t *testing.T) {
			src := `Процедура Тест()
				Д = Новый Структура;
				Д.Вставить("Дата", Дата(2026, 8, 17));
				Д.Вставить("Сумма", 1234.5);
				Возврат Новый ШаблонHTML("` + expr + `").Заполнить(Д);
			КонецПроцедуры`
			assert.Equal(t, want[name], evalTemplate(t, src))
		})
	}
}
