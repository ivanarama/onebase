...
func (p SandboxProfile) Vars() map[string]any {
	...
	// Приостановить не запрещается — ждать безопасно, — но её предел опускается
	// до wall-clock профиля. Иначе она была бы единственным способом занять
	// поток дольше отведённого: лимиты профиля (дедлайн, итерации) проверяются
	// МЕЖДУ операторами, а спящий оператор до проверки не доходит, и
	// `Приостановить(300)` держал бы сессию пять минут при лимите в десять
	// секунд — дыра ровно того же класса, что закрывают запреты выше.
	if p.MaxWallClock > 0 {
		for k, v := range NewSleepFunctions(p.MaxWallClock) {
			m[k] = v
		}
	}
	return m
