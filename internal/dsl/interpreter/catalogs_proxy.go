...
// ErrRowAccessDenied — отказ прав доступа к данным. Текст читает пользователь:
// он всплывает в интерфейсе как результат обработки, и «row access denied»
// говорило ему только то, что что-то сломалось. Сравнения идут через
// errors.Is по идентичности, так что формулировка ни на что не влияет.
