...
// по-прежнему Ложь, `nil = ""` и `nil = nil` сравниваются как раньше, а ключи
// Соответствия правило не задевает вовсе: findIdx ходит через refKey, и
// «нет значения» остаётся ключом, отличным от нуля.
