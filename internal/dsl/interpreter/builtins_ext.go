...
func isBlankVal(v any) bool {
	...
	case time.Time:
		...
		// Сравниваем по календарной дате, а не через IsZero: та верна только для
		// UTC, а «пустая» дата приезжает и в локальной зоне (разбор строки,
		// драйвер БД) — там IsZero уже ложь, и проверка снова молчала бы.
		y, mo, d := t.Date()
